...
class GridWorld:
    terrain_color = dict(normal=[127 / 360, 0, 96 / 100],
                         ...
                         player=[344 / 360, 93 / 100, 100 / 100])   # 字典：普通格子、目的地、悬崖、玩家，3 channels 着色数据
    ...
    def _create_grid(self, initial_grid=None):
        self.grid = self.terrain_color['normal'] * np.ones((4, 12, 3))  # np.ones(shape, dtype, order)，所以两层括号
        # 全部初始化为normal的对应着色
        self._add_objectives(self.grid)

    def _add_objectives(self, grid):
        grid[-1, 1:11] = self.terrain_color['cliff']    # grid[最下面一排，1：11，全部channels] = cliff对应着色
        grid[-1, -1] = self.terrain_color['objective']  # grid[最下面一排，最后一列，全部channels] = objective对应着色
