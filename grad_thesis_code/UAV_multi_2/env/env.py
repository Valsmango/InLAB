...
class Env(object):
    def __init__(self):
        ...
        self.marker_list = ['x', '.', '|']
        self.color_list = [(46 / 255, 139 / 255, 87 / 255), (106 / 255, 90 / 255, 205 / 255),
    ...
    def get_path_delay(self):
        final_tar_dis = 0.
        for i in range(self.n_uav):
            final_tar_dis += np.sqrt((self.uav_state[i][0] - self.target[i][0]) ** 2 +
        ...
        for i in range(self.n_uav):
            ...
            for j in range(path_len - 1):
                plt.scatter([self.path[i][j][0] / 1000], [self.path[i][j][1] / 1000],
                            color=self.color_list[i], alpha=0.3, marker=self.marker_list[i])
