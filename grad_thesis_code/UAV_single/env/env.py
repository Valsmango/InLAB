...
class Env(object):
    ...
    def get_path_delay(self):
        final_tar_dis = np.sqrt((self.uav_state[0][0] - self.target[0][0]) ** 2 +
                          (self.uav_state[0][1] - self.target[0][1]) ** 2 +
                          (self.uav_state[0][2] - self.target[0][2]) ** 2)
        return final_tar_dis
    ...
    def plot_env(self):
        ...
        ax.scatter(static_xscatter, static_yscatter, static_zscatter, label='static obstacle',
                   color="k", alpha=1)
        ...
        #               alpha=0.3, color=[1.0, 0.35, 0.35], linestyle=':')
