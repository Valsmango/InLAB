...
class Env(object):
    def __init__(self):
        ...
        startB=[2500,0,150];
        endB=[2500,5000,150];
        course_angle = [0;1/2*pi];
        
        % case 3：追及
        % startA=[0,2500,150];
        % endA=[5000,2500,150];
        % startB=[1250,2500,150];
        % endB=[3250,2500,150
        '''
        # 逆向
        # init_start = [
        ...
        #      4800, 2500, 150],
        #     [4800, 2500, 150, -100, 0, 0, 200, 2500, 150,
        #      200, 2500, 150]]
        # init_target = [[4800, 2500, 150], [200, 2500, 150]]
        ...
        #     [200, 2500, 150, 100, 0, 0, 4800, 2500, 150,
        #      1250, 2500, 150],
        #     [1250, 2500, 150, 0, 43.5, 0, 3250, 2500, 150,
        #      200, 2500, 150]]
        # init_target = [[4800, 2500, 150], [3250, 2500, 150]]
        # 修改既定冲突点
        init_start = [
            [200, 2500, 150, 100, 0, 0, 4800, 2500, 150,
             1250, 2500, 150],
    ...
    def get_path_len(self):
        ...
        for i in range(self.n_uav):
            for j in range(len(self.path[i])):
                if j == 0:
                    path_length += np.sqrt(self.path[i][j][0]**2 + self.path[i][j][1]**2 + self.path[i][j][2]**2)
                else:
                    path_length += np.sqrt((self.path[i][j][0]-self.path[i][j-1][0])**2 +
                                         (self.path[i][j][1]-self.path[i][j-1][1])**2 +
                                         (self.path[i][j][2]-self.path[i][j-1][2])**2)
        return path_length

    def get_path_delay(self):
        tar_dis = 0.
        for i in range(self.n_uav):
            tar_dis += np.sqrt((self.uav_state[i][0] - self.target[i][0]) ** 2 +
    ...
    def step(self, input_action):
        ...
        for i in range(self.n_uav):
            if self.continue_flag[i]:
                ...
                self.uav_state[i] = [0. for _ in range(12)]
    ...
    def _show_3D_path(self):
        ...
        for i in range(self.n_uav):
            ...
            z = [self.path[i][j][2] for j in range(path_len)]
            ax.scatter([self.path[i][path_len - 1][0]], [self.path[i][path_len - 1][1]],
                       [self.path[i][path_len - 1][2]], color=color_list[i], alpha=0.7, label=f"UAV {i + 1}")
            # for j in range(path_len - 1):
            #     ax.scatter([self.path[i][j][0] / 1000], [self.path[i][j][1] / 1000],
            #                [self.path[i][j][2] / 1000], color='green', alpha=0.3)

            ax.plot3D(x, y, z, color=color_list[i])
            ax.plot3D([self.uav_init_state[i][0], self.target[i][0]],
                      [self.uav_init_state[i][1], self.target[i][1]],
                      [self.uav_init_state[i][2], self.target[i][2]],
                      color=color_list[i], alpha=0.3, linestyle=':')
    ...
    def _show_xy_path(self):
        ...
        for i in range(self.n_uav):
            ...
            plt.scatter([self.path[i][path_len - 1][0]], [self.path[i][path_len - 1][1] ],
                        color=color_list[i], alpha=0.7, label=f"UAV {i + 1}")
            # for j in range(path_len - 1):
