...
if __name__ == '__main__':
    ...
    for t in range(int(max_train_steps)):
        ...
        if t < random_steps:  # Take random actions in the beginning for the better exploration
            ...
            a = [(agent[i].choose_action(s[i], h[i], c[i]) +
        ...
        for i in range(n_agents):
            ...
            episode_record[i].store(s[i], a[i], r[i], s_[i], dw, store_flags[i])  # Store the transition
        s = s_
        ...
        if np.all(np.array(done)):
            ...
            # Reset environment
            replay_buffer[i].put(episode_record[i])
            env.close()
            env = Env()
