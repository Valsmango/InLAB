...
'''
设计5个场景：
1，【150，150，150】 --> 【4850，4850，150】
2，【150，150，150】 --> 【4850，150，150】
3， 
4，
5，
...
class EvalEnv(object):
    ...
    def get_path_len(self):
        ...
        for i in range(len(self.path)):
            if i == 0:
                path_length += np.sqrt(self.path[i][0]**2 + self.path[i][1]**2 + self.path[i][2]**2)
            else:
                path_length += np.sqrt((self.path[i][0]-self.path[i-1][0])**2 +
                                     (self.path[i][1]-self.path[i-1][1])**2 +
                                     (self.path[i][2]-self.path[i-1][2])**2)
        return path_length
    ...
    def _show_3D_path(self):
        ...
        static_obs_len = len(self.static_obs_state)
        static_xscatter = [self.static_obs_state[i]['x'] for i in range(static_obs_len)]
        static_yscatter = [self.static_obs_state[i]['y'] for i in range(static_obs_len)]
        static_zscatter = [self.static_obs_state[i]['z'] for i in range(static_obs_len)]
        ax.scatter(static_xscatter, static_yscatter, static_zscatter, label='static obstacle', c='black', alpha=0.7)

        dynamic_obs_len = len(self.dynamic_obs_init_state)
        dynamic_xscatter = [self.dynamic_obs_init_state[i]['x']  for i in range(dynamic_obs_len)]
        dynamic_yscatter = [self.dynamic_obs_init_state[i]['y'] for i in range(dynamic_obs_len)]
        dynamic_zscatter = [self.dynamic_obs_init_state[i]['z'] for i in range(dynamic_obs_len)]
        # ax.scatter(dynamic_xscatter, dynamic_yscatter, dynamic_zscatter, c='r', alpha=0.3)

        dynamic_obs_cur_len = len(self.dynamic_obs_state)
        dynamic_xscatter_cur = [self.dynamic_obs_state[i]['x'] for i in range(dynamic_obs_cur_len)]
        dynamic_yscatter_cur = [self.dynamic_obs_state[i]['y']  for i in range(dynamic_obs_cur_len)]
        dynamic_zscatter_cur = [self.dynamic_obs_state[i]['z']  for i in range(dynamic_obs_cur_len)]
        ax.scatter(dynamic_xscatter_cur, dynamic_yscatter_cur, dynamic_zscatter_cur, label='dynamic obstacle',
                   c='r', alpha=0.7)
        ...
        for k in range(dynamic_obs_len):
            ax.plot3D([dynamic_xscatter_cur[k], dynamic_xscatter[k]],
                      [dynamic_yscatter_cur[k], dynamic_yscatter[k]],
                      [dynamic_zscatter_cur[k], dynamic_zscatter[k]],
                      alpha=0.2, c='r', linestyle=':')

        path_len = len(self.path)
        x = [self.path[i][0] for i in range(path_len)]
        y = [self.path[i][1] for i in range(path_len)]
        z = [self.path[i][2] for i in range(path_len)]
        ax.scatter([self.path[path_len - 1][0]], [self.path[path_len - 1][1]],
                   [self.path[path_len - 1][2]], color='green', alpha=0.7, label='UAV')
        ax.plot3D(x, y, z, color='green')
        for k in range(len(self.uav_init_state)):
            ax.plot3D([self.uav_init_state[k][0], self.target[k][0]],
                      [self.uav_init_state[k][1], self.target[k][1]],
                      [self.uav_init_state[k][2], self.target[k][2]],
    ...
    def _show_xy_path(self):
        ...
        x = [self.path[i][0] for i in range(path_len)]
        y = [self.path[i][1] for i in range(path_len)]
        plt.scatter([self.path[path_len - 1][0]], [self.path[path_len - 1][1]],
                    color='green', alpha=0.7, label='UAV')
        plt.plot(x, y, color='green')
        for k in range(len(self.uav_init_state)):
            plt.plot([self.uav_init_state[k][0], self.target[k][0]],
                     [self.uav_init_state[k][1], self.target[k][1]],
                     color='green', alpha=0.3, linestyle=':')

        circle = plt.Circle(xy=([self.path[path_len - 1][0]], [self.path[path_len - 1][1]]),
                            radius=self.min_sep_hori, linestyle=':', color='green', fill=False)
