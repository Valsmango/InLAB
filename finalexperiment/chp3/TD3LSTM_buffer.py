...
class EpisodeMemory():
    ...
    def __init__(self, random_update=False,
        ...
        self.success_size = int(self.max_epi_num * 0.5)
...
class TD3LSTM(object):
    ...
    def learn(self, relay_buffer):
        ...
        ratio = [0.0, 0.0]
        batch_s, batch_a, batch_r, batch_s_, batch_dw = relay_buffer.sample(self.batch_size, ratio)  # Sample a batch
...
if __name__ == '__main__':
    ...
    if not os.path.exists("eval_reward_train/TD3LSTM_buffer"):
        os.makedirs("./eval_reward_train/TD3LSTM_buffer")
    if not os.path.exists("./model_train/TD3LSTM_buffer"):
        os.makedirs("./model_train/TD3LSTM_buffer")
    ...
    # for t in range(int(max_train_steps)):
        ...
        if done:
            ...
            if (episode_num + 1) % 100 == 0:
                np.save('./eval_reward_train/TD3LSTM_buffer/train_reward_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_rewards))
                np.save('./eval_reward_train/TD3LSTM_buffer/train_ma_reward_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_ma_rewards))
                np.save('./eval_reward_train/TD3LSTM_buffer/train_success_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_success_rate))
                np.save('./eval_reward_train/TD3LSTM_buffer/train_ma_success_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_ma_success_rate))
                np.save('./eval_reward_train/TD3LSTM_buffer/train_collision_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_collision_rate))
                np.save('./eval_reward_train/TD3LSTM_buffer/train_ma_collision_TD3LSTM_buffer_env_{}_seed_{}.npy'.format(env_name, seed),
            ...
            episode_timesteps = 0
            episode_reward = 0.
            replay_buffer.put(episode_record, category, episode_reward)

        t += 1

    agent.save(f"./model_train/TD3LSTM_buffer/TD3LSTM_buffer_{env_name}")
    end_time = time.time()
