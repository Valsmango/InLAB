...
class Critic(nn.Module):  # According to (s,a), directly calculate Q(s,a)
    def __init__(self, state_dim, action_dim, hidden_width):
        ...
        self.l2 = nn.Linear(hidden_width, hidden_width)
        self.l3 = nn.LSTM(hidden_width, hidden_width, bidirectional=True, batch_first=True)
        self.l4 = nn.Linear(hidden_width*2, 1)
        # orthogonal_init(self.l1)
        # orthogonal_init(self.l2)
        ...
        self.l7 = nn.LSTM(hidden_width, hidden_width, bidirectional=True, batch_first=True)
        self.l8 = nn.Linear(hidden_width*2, 1)
    ...
    def init_hidden_state(self, batch_size, training=None):
        ...
        '''
        if training is True:
            return torch.zeros([2, batch_size, self.hidden_width]), torch.zeros([2, batch_size, self.hidden_width])
        else:
            return torch.zeros([2, 1, self.hidden_width]), torch.zeros([2, 1, self.hidden_width])
...
class TD3LSTM(object):
    ...
    def learn(self, relay_buffer):
        ...
        with torch.no_grad():  # target_Q has no gradient
            ...
            noise = (torch.randn_like(batch_a) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)    #    【batch，len, dim】
