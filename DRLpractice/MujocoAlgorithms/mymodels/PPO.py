...
class ActorNetwork(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.Linear1(x))  # 有的用的tanh进行激活
        x = F.relu(self.Linear2(x))
        mu = self.Linear3(x)     # 连续动作空间不需要softmax，而是要改进成输出一个均值mu
...
class PPO:
    def __init__(self, batch_size, state_space, action_space, clip,
        ...
        self.memory = myutils.PPOReplayBuffer()
        self.actor = ActorNetwork(state_space=self.state_space, action_space=self.action_space).to(device)
    ...
    def update(self):
        ...
        for _ in range(self.epochs):
            state, action, reward, done, old_prob, value, batches = self.memory.sample(self.batch_size)
            # GAE
