...
if __name__ == "__main__":
    ...
    if not os.path.exists("./eval_reward_train/MADDPG_COMMON/"):
        os.makedirs("./eval_reward_train/MADDPG_COMMON/")

    if not os.path.exists("./model_train/MADDPG_COMMON/"):
        os.makedirs("./model_train/MADDPG_COMMON/")
        ...
        if np.all(np.array(dones)):
            ...
            if train_episode_ma_rewards:
                train_episode_ma_rewards.append(
                    0.99 * train_episode_ma_rewards[-1] + 0.01 * episode_reward)  # 移动平均，每50个episode的
            ...
            if (episode_num + 1) % 100 == 0:
                np.save('./eval_reward_train/MADDPG_COMMON/train_reward_MADDPG_COMMON_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_rewards))
                np.save('./eval_reward_train/MADDPG_COMMON/train_ma_reward_MADDPG_COMMON_env_{}_seed_{}.npy'.format(env_name, seed),
    ...
    env.close()
    maddpg.save(f"./model_train/MADDPG_COMMON/MADDPG_COMMON_{env_name}")
