...
if __name__ == '__main__':
    ...
    if not os.path.exists("./eval_reward_train/RATD3/"):
        os.makedirs("./eval_reward_train/RATD3/")
    if not os.path.exists("model_train/RATD3/"):
        os.makedirs("./model_train/RATD3/")
    ...
    # for t in range(int(max_train_steps)):
        ...
        if np.all(np.array(done)):
            ...
            if (episode_num + 1) % 100 == 0:
                np.save('./eval_reward_train/RATD3/train_reward_TD3_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_rewards))
                np.save('./eval_reward_train/RATD3/train_ma_reward_TD3_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_ma_rewards))
                np.save('./eval_reward_train/RATD3/train_success_TD3_env_{}_seed_{}.npy'.format(env_name, seed),
                        np.array(train_episode_success_rate))
                np.save('./eval_reward_train/RATD3/train_ma_success_TD3_env_{}_seed_{}.npy'.format(env_name, seed),
    ...
    env.close()
    for i in range(n_agents):
        agent[i].save(f"./model_train/RATD3/agent_{i}_TD3_{env_name}")
    end_time = time.time()
