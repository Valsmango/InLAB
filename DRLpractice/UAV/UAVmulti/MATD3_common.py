...
class TD3Agent(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr)  # 1e-3
        # self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=self.lr) # 1e-4
...
class MATD3(object):
    def __init__(self, n_agents, state_dim, action_dim, max_action):
        ...
        self.max_action = max_action
        self.policy_noise = 0.4 * self.max_action  # The noise for the trick 'target policy smoothing'
        self.noise_clip = 0.5 * self.max_action  # Clip the noise
        self.noise_std = 0.2 * self.max_action
        self.action_dim = action_dim
        self.state_dim = state_dim
        self.actor_pointer = 0
        self.common_critic = Critic(state_dim, action_dim, 256).to(device)
        self.common_critic_target = copy.deepcopy(self.common_critic)
        self.common_critic_optimizer = torch.optim.Adam(self.common_critic.parameters(), lr=3e-4)
    ...
    def debug_sample(self):
        obs = torch.tensor([[[0.3831, 0.7087, 0.3073, -0.3650, 0.4530, 0.3744, 0.3214,
                              0.9426, 0.5364, 0.6414, 0.6072, 0.5980, 0.6518, 0.3442,
                              0.8007],
                             [0.2629, 0.0983, 0.1800, 0.2088, 0.4618, 0.3282, 0.7454,
                              0.9201, 0.8309, 0.8424, 0.8119, 0.3386, 0.9657, 0.4779,
                              0.4996]],
                            [[0.6414, 0.6072, 0.5980, 0.5009, 0.1880, -0.2276, 0.9034,
                              0.6876, 0.1735, 0.6518, 0.3442, 0.8007, 0.3831, 0.7087,
                              0.3073],
                             [0.8424, 0.8119, 0.3386, -0.3580, -0.3246, 0.1451, 0.1433,
                              0.1751, 0.6662, 0.9657, 0.4779, 0.4996, 0.2629, 0.0983,
                              0.1800]],
                            [[0.6518, 0.3442, 0.8007, 0.3734, -0.2833, 0.7033, 0.9494,
                              0.5487, 0.7062, 0.3831, 0.7087, 0.3073, 0.6414, 0.6072,
                              0.5980],
                             [0.9657, 0.4779, 0.4996, -0.4482, 0.0691, 0.0938, 0.0164,
                              0.5248, 0.5035, 0.2629, 0.0983, 0.1800, 0.8424, 0.8119,
                              0.3386]]]).to(device)

        acs = torch.tensor([[[0.0752, 0.5938, -0.5792],
                             [-0.9292, -0.6972, 0.6461]],
                            [[0.6790, 0.1677, 0.5554],
                             [-0.8635, 0.9198, 0.3565]],
                            [[0.4635, -0.4355, -0.0763],
                             [0.5847, -0.5393, 0.1955]]]).to(device)
        rews = torch.tensor([[[-0.7415],
                              [-1.4865]],
                             [[-0.7771],
                              [-1.4767]],
                             [[-0.8900],
        ...
        next_obs = torch.tensor([[[0.3686, 0.7281, 0.3159, -0.3612, 0.4827, 0.2590, 0.3214,
    ...
    def update(self, sample, agent_i, target_update):
        ...
        # obs, acs, rews, next_obs, dones = self.debug_sample()
        ...
        with torch.no_grad():
            ...
            Q1_next, Q2_next = self.common_critic(next_obs, all_trgt_acs)  # Q1和Q2都是tensor([2,64,1])
        ...
        current_Q1, current_Q2 = self.common_critic(obs, acs)  # Q1和Q2都是tensor([2,64,1])
        critic_loss = self.MseLoss(current_Q1, all_trgt_Q.detach()) + self.MseLoss(current_Q2, all_trgt_Q.detach())
        # Optimize the critic
        self.common_critic_optimizer.zero_grad()
        critic_loss.backward()
        nn.utils.clip_grad_norm_(self.common_critic.parameters(), 0.5)
        ...
        if (self.actor_pointer - 1) % (self.policy_freq * self.n_agents) == agent_i:
            ...
            actor_loss = -self.common_critic.Q1(obs, torch.Tensor(all_pol_acs).to(device)).mean()
            # actor_loss += -self.common_critic.Q1(obs[agent_i].unsqueeze(0),
            #                                curr_agent.actor(obs[agent_i]).unsqueeze(0)).mean()
            actor_loss += (curr_pol_out ** 2).mean() * 1e-3

            ############################ 更新方式2：个体最优 ####################################
            # 使agent自身的Q最大，即以个体最优为目标：
            #    actor_loss = -self.common_critic.Q1(obs[agent_i].unsqueeze(0), curr_agent.actor(obs[agent_i]).unsqueeze(0)).mean()
            ...
            # actor_loss = -self.common_critic.Q1(extra_self_obs, torch.Tensor(extra_self_acs).to(device)).mean()
...
class MATD3ReplayBuffer(object):
    ...
    def sample(self, batch_size):
        ...
        batch_obs = torch.tensor([self.observations[agent_i][index] for agent_i in range(self.n_agents)],
                                 dtype=torch.float).to(device)
        batch_a = torch.tensor([self.actions[agent_i][index] for agent_i in range(self.n_agents)],
                               dtype=torch.float).to(device)
        batch_r = torch.tensor([self.rewards[agent_i][index] for agent_i in range(self.n_agents)],
                               dtype=torch.float).to(device)
        batch_next_obs = torch.tensor([self.next_observations[agent_i][index] for agent_i in range(self.n_agents)],
                                      dtype=torch.float).to(device)
        batch_dw = torch.tensor([self.dones[agent_i][index] for agent_i in range(self.n_agents)], dtype=torch.float).to(
            device)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./eval_reward_train/MATD3_COMMON/"):
        os.makedirs("./eval_reward_train/MATD3_COMMON/")

    if not os.path.exists("./model_train/MATD3_COMMON/"):
        os.makedirs("./model_train/MATD3_COMMON/")

    matd3 = MATD3(n_agents=n_agents, state_dim=state_dim, action_dim=action_dim, max_action=max_action)
    replay_buffer = MATD3ReplayBuffer(n_agents=n_agents, state_dim=state_dim, action_dim=action_dim,
                                      max_size=int(max_timesteps))
    ...
    while episode_num < max_train_episodes:
        ...
        if t >= start_timesteps:
            ...
            for i in range(n_agents):
                ...
                critic_loss = matd3.update(sample, i, update_target)
        ...
        if np.all(np.array(dones)):
            ...
            if (episode_num + 1) % 100 == 0:
                np.save('./eval_reward_train/MATD3_COMMON/train_reward_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name,
                                                                                                               seed),
                        np.array(train_episode_rewards))
                np.save(
                    './eval_reward_train/MATD3_COMMON/train_ma_reward_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name,
                                                                                                              seed),
                    np.array(train_episode_ma_rewards))
                np.save(
                    './eval_reward_train/MATD3_COMMON/train_success_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name,
                                                                                                            seed),
                    np.array(train_episode_success_rate))
                np.save(
                    './eval_reward_train/MATD3_COMMON/train_ma_success_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name,
                                                                                                               seed),
        ...
        t += 1
    env.close()
    np.save('./eval_reward_train/MATD3_COMMON/train_reward_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name, seed),
            np.array(train_episode_rewards))
    np.save('./eval_reward_train/MATD3_COMMON/train_ma_reward_MATD3_COMMON_env_{}_seed_{}.npy'.format(env_name, seed),
            np.array(train_episode_ma_rewards))
    matd3.save(f"./model_train/MATD3_COMMON/MATD3_COMMON_{env_name}")
